...
// ── CDN loader ────────────────────────────────────────────────────────────
// CRITICAL: All @codemirror/* packages MUST share the same instance of
// @codemirror/state and @codemirror/view, otherwise instanceof checks fail
// with "Unrecognized extension value in extension set".
//
// Solution: use esm.sh's ?external flag to prevent each package from
// bundling its own copy of the core deps, then load core deps first and
// let the rest resolve against the already-cached versions.
//
// Even simpler: load via a single inline module script that uses a single
// esm.sh bundle URL with all packages listed, ensuring one resolution graph.

async function loadDeps() {
  console.log('[NvimEditor] Loading CodeMirror + Vim via dynamic import()...');

  // Use esm.sh's ?bundle mode for the core packages to ensure a single
  // dependency graph. We load @codemirror/state first, then everything
  // else pins to that exact version using ?deps=.
  const STATE_VER  = '@codemirror/state@6.5.0';
  const VIEW_VER   = '@codemirror/view@6.34.3';
  const LANG_VER   = '@codemirror/language@6.10.8';
  const CMDS_VER   = '@codemirror/commands@6.7.1';

  // The ?deps= param tells esm.sh to use the specified versions of peer
  // dependencies instead of resolving its own, guaranteeing a single copy.
  const SHARED = `?deps=${STATE_VER},${VIEW_VER},${LANG_VER},${CMDS_VER}`;
  const ESM = 'https://esm.sh';

  try {
    // Step 1: Load core modules first (state, view) — these are the shared singletons
    const stateMod = await import(/* webpackIgnore: true */ `${ESM}/${STATE_VER}`);
    const viewMod  = await import(/* webpackIgnore: true */ `${ESM}/${VIEW_VER}?deps=${STATE_VER}`);

    // Step 2: Load everything else pinned to the same core versions
    const [cmdsMod, searchMod, autoMod, langMod, mdMod, jsMod, pyMod, jsonMod, htmlMod, cssMod, vimMod] =
      await Promise.all([
        import(/* webpackIgnore: true */ `${ESM}/${CMDS_VER}${SHARED}`),
        import(/* webpackIgnore: true */ `${ESM}/@codemirror/search@6.5.8${SHARED}`),
        import(/* webpackIgnore: true */ `${ESM}/@codemirror/autocomplete@6.18.4${SHARED}`),
        import(/* webpackIgnore: true */ `${ESM}/${LANG_VER}?deps=${STATE_VER}`),
        import(/* webpackIgnore: true */ `${ESM}/@codemirror/lang-markdown@6.3.2${SHARED}`),
        import(/* webpackIgnore: true */ `${ESM}/@codemirror/lang-javascript@6.2.2${SHARED}`),
        import(/* webpackIgnore: true */ `${ESM}/@codemirror/lang-python@6.1.6${SHARED}`),
        import(/* webpackIgnore: true */ `${ESM}/@codemirror/lang-json@6.0.1${SHARED}`),
        import(/* webpackIgnore: true */ `${ESM}/@codemirror/lang-html@6.4.9${SHARED}`),
        import(/* webpackIgnore: true */ `${ESM}/@codemirror/lang-css@6.3.1${SHARED}`),
        import(/* webpackIgnore: true */ `${ESM}/@replit/codemirror-vim@6.2.1${SHARED}`),
      ]);
    ...
    window.__CM = {
      ...
      gutter: viewMod.gutter,
      GutterMarker: viewMod.GutterMarker,
      // state
    ...
    console.log('[NvimEditor] All modules loaded ✓ — vim:', typeof window.__CM.vim);
    window.dispatchEvent(new Event('cm-ready'));
  } catch (err) {
    console.error('[NvimEditor] Failed to load modules:', err);
    throw err;
  }
}
...
function injectNvimEditor() {
  ...
  if (!window.__CM) {
    host.querySelector('.nvim-cm-wrap').innerHTML =
      '<div style="padding:20px;color:#f38ba8;font-family:monospace;font-size:13px;">' +
      '⏳ Loading Vim engine from CDN…<br>' +
      '<span style="color:#6c7086;font-size:11px;">If this persists, check DevTools console (Ctrl+Shift+I) for errors.</span></div>';
    return;
  }
...
app.renderEditor = function() {
  ...
  if (!window.__CM) {
    // Deps still loading — listen for ready event.
    // Remove any previous listener to avoid duplicates.
    if (window.__nvimPendingListener) {
      window.removeEventListener('cm-ready', window.__nvimPendingListener);
    }
    window.__nvimPendingListener = () => {
      window.__nvimPendingListener = null;
      setTimeout(injectNvimEditor, 50);
    };
    window.addEventListener('cm-ready', window.__nvimPendingListener, { once: true });
  } else {
    setTimeout(injectNvimEditor, 50);
  }
};
...
(async () => {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES && !loaded; attempt++) {
    try {
      console.log(`[NvimEditor] Loading dependencies (attempt ${attempt}/${MAX_RETRIES})...`);
      await loadDeps();
      loaded = true;
      console.log('[NvimEditor] CodeMirror + Vim loaded ✓');
    } catch (err) {
      console.error(`[NvimEditor] Attempt ${attempt} failed:`, err);
      if (attempt === MAX_RETRIES) {
        console.error('[NvimEditor] All attempts failed. Vim mode will not be available.');
        console.error('[NvimEditor] The editor will fall back to the standard textarea.');
        // Show a subtle warning in the status bar area if possible
        const statusRight = document.querySelector('.status-right');
        if (statusRight) {
          const warn = document.createElement('span');
          warn.style.cssText = 'color:#f38ba8;font-size:11px;margin-right:8px;';
          warn.textContent = '⚠ Vim mode failed to load';
          warn.title = 'CodeMirror CDN modules could not be loaded. Check DevTools console.';
          statusRight.prepend(warn);
        }
        return;
      }
